import {RRVPlatform} from '../../../typechain-types/RRVPlatform';
import {deploy, evm_revert, evm_snapshot} from '../../utils/hardhat-helpers';
import {loadFixture} from '@nomicfoundation/hardhat-network-helpers';
import {expect} from 'chai';
import {ethers, network} from 'hardhat';
import {RoyaltyConfig, TokenMetadataConfig} from '../../utils/types';

describe('RRV Platform', function () {
  async function deployFixture() {
    ...
    const artzoneContract = await deploy<RRVPlatform>('RRVPlatform', [
      'Artzone Collections',
      'Artzone Collections',
      100,
    ]);
  ...
  describe('[Token Initialisation]', function () {
    it('should be able to initialise single token', async function () {
      ...
      const token1MintTx = await artzoneContract
        .connect(owner)
        .initialiseNewSingleToken(TEST_TOKEN_1, otherAccount1.address);
    ...
    it('should be able to initialise multiple single tokens via `initialiseNewMultipleTokens', async function () {
      ...
      const tokensMintTx = await artzoneContract
        .connect(owner)
        .initialiseNewMultipleTokens(tokensToInitialise, revenueRecipients);
  ...
  describe('[Token Mint]', function () {
    it('should be able to mint batch tokens and process fees accordingly', async function () {
      ...
      expect(balanceWeiOfAcc1After.sub(balanceWeiOfAcc1Before).toString()).to.be.equal(
        ethers.BigNumber.from(198).toString(),
      );

      // Expect Acc2 to increase by (100 * 990) / 10_000 = 99
      expect(balanceWeiOfAcc2After.sub(balanceWeiOfAcc2Before).toString()).to.be.equal(
        ethers.BigNumber.from(0).toString(),
      );

      // Expect Acc3 to increase by (100 * 990) / 10_000 = 99
      expect(balanceWeiOfAcc3After.sub(balanceWeiOfAcc3Before).toString()).to.be.equal(
        ethers.BigNumber.from(99).toString(),
      );

      // expect Artzone Contract to gain (3 * 100 * 100) / 10_000 = 3
      const expectedArtzoneFundsReceived = ethers.BigNumber.from(3);
      expect(artzoneBalanceAfter.toString()).to.be.equal(expectedArtzoneFundsReceived.toString());

      const account4BalanceBefore = await provider.getBalance(otherAccount4.address);

      // Withdraw funds by owner to `otherAccount4`
      const withdrawFundsTx = await artzoneContract.connect(owner).collectPlatformFees(otherAccount4.address);
      await withdrawFundsTx.wait();

      const account4BalanceAfter = await provider.getBalance(otherAccount4.address);
      expect(account4BalanceAfter.sub(account4BalanceBefore).toString()).to.be.equal(
        expectedArtzoneFundsReceived.toString(),
      );
    });
  });
});
